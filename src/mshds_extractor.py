...
def _speechrate(snd):
    ...
    try:
        # Hyperparameters based on the provided notebook
        silencedb = -25
        mindip = 2
        minpause = 0.3
...
def _pitch_values(snd):
    """Estimates pitch floor and ceiling for a robust search."""
    try:
        ...
        if len(pitch_values_arr) == 0: return 75, 500
        ...
        if mean_pitch < 170:
            pitch_floor, pitch_ceiling = 60, 250
        else:
...
def _extract_pitch(snd, floor, ceiling, frame_shift):
    """Extracts mean F0 and stdev F0 in semitones."""
    try:
...
def _extract_intensity(snd, floor, frame_shift):
    """Extracts mean intensity and intensity range."""
    try:
...
def _extract_harmonicity(snd, floor, ceiling, frame_shift):
    """Extracts mean Harmonics-to-Noise Ratio (HNR)."""
    try:
...
def _extract_Slope_Tilt(snd, floor, ceiling):
    """Extracts spectral slope and tilt."""
    try:
...
def _extract_CPP(snd, floor, ceiling, frame_shift):
    """Extracts mean Cepstral Peak Prominence (CPP) with debugging prints."""
    
    try:
        # These definitions are now inside the main try block
        ...
        CPP_list = []
        all_cpp_values_before_filtering = [] # New list for debugging
        ...
        for i in range(1, n_intervals + 1):
            ...
            if label == 'V':
                ...
                # Check for zero-duration segments which can cause errors
                if tmin >= tmax:
                    continue
        ...
        # This will catch errors in the initial setup (pitch, pulses, vuv_table)
        print(f"  > Main CPP function failed with error: {e_main}")
        return np.nan

def _measureFormants(snd, floor, ceiling, frame_shift):
    """Extracts features for the first two formants."""
    try:
...
def _extract_Spectral_Moments(snd, floor, ceiling, window_size, frame_shift):
    """Extracts the first four spectral moments."""
    try:
        pitch = snd.to_pitch_ac(time_step=frame_shift, pitch_floor=floor, pitch_ceiling=ceiling)
